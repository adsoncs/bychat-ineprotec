import { Wand2, Undo2 } from 'lucide-preact'
  ...
  return (
    <Card class="p-3 border-success/40 bg-success/10">
      <div class="text-xs uppercase tracking-wider text-success font-semibold mb-2 flex items-center gap-1">
        ...
        {data.at && <span class="text-fg-subtle normal-case font-normal">· {formatRelative(data.at)}</span>}
      </div>
      <div class="space-y-1.5">
        {active.map((p) => (
          <div key={p.field} class="flex items-center gap-2 text-xs">
            <span class="text-fg-subtle w-16 shrink-0">{FIELD_LABELS[p.field] ?? p.field}:</span>
            ...
            <button
              type="button"
              class="inline-flex items-center gap-1 h-6 px-2 rounded text-[0.6875rem] font-medium border border-border bg-surface text-fg-muted hover:text-danger hover:border-danger/40 disabled:opacity-40"
